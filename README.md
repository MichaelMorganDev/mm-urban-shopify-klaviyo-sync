...
### Klaviyo customer metafield sync

`metafields/update` **cannot** be declared in `shopify.app.toml` for app versions (Partner/CLI will reject it). This app instead registers a **shop-specific** [`CUSTOMERS_UPDATE`](https://shopify.dev/docs/api/admin-graphql/latest/enums/WebhookSubscriptionTopic#value-customersupdate) webhook with `metafieldNamespaces` `counterpoint` and `klaviyo` (see [`app/shopify.server.ts`](app/shopify.server.ts)). On each eligible customer update, [`app/routes/webhooks.customers.update.tsx`](app/routes/webhooks.customers.update.tsx) looks up the **existing** Klaviyo profile by Shopify customer email and **PATCHes** custom properties (**no profile creation**; if no profile exists, the webhook is skipped with a log). It makes **one Klaviyo PATCH per mapped metafield** after the lookup. If the webhook body has no inline `metafields` (common), it **loads `counterpoint` / `klaviyo` metafields via the Admin API** instead (see [`app/lib/klaviyoPropertyMap.ts`](app/lib/klaviyoPropertyMap.ts)).

**After deploy or code changes**, open the app once per shop (or reinstall) so `afterAuth` runs and the subscription is registered.
...
1. Run `shopify app dev`, install the app on a dev store, and ensure the app requests `read_customers` (there is no separate `read_metafields` scope for Partner apps—customer data and webhook metafield payloads use this scope).
2. In Admin, set a customer metafield (e.g. `counterpoint` / `LastSaleDate`) on a customer that has a **default email**.
3. Confirm the corresponding Klaviyo property (e.g. **Last Sale Date**) updates for that email.
4. Optional: `shopify app webhook trigger --topic CUSTOMERS_UPDATE --address <your-local-or-staging-url>` (sample payload may omit metafields; use real customer metafield edits for end-to-end tests).

**If “nothing happens” when you edit a metafield**
...
3. Responding to webhooks in individual files such as [/app/routes/webhooks.app.uninstalled.tsx](https://github.com/Shopify/shopify-app-template-remix/blob/main/app/routes/webhooks.app.uninstalled.tsx), [/app/routes/webhooks.app.scopes_update.tsx](https://github.com/Shopify/shopify-app-template-remix/blob/main/app/routes/webhooks.app.scopes_update.tsx), and this repo’s [`app/routes/webhooks.customers.update.tsx`](app/routes/webhooks.customers.update.tsx) (Klaviyo sync).
