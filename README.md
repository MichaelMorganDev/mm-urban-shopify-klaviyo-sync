...
`metafields/update` **cannot** be declared in `shopify.app.toml` for app versions (Partner/CLI will reject it). This app instead registers a **shop-specific** [`CUSTOMERS_UPDATE`](https://shopify.dev/docs/api/admin-graphql/latest/enums/WebhookSubscriptionTopic#value-customersupdate) webhook with `metafieldNamespaces` `counterpoint` and `klaviyo` (see [`app/shopify.server.ts`](app/shopify.server.ts)). On each eligible customer update, [`app/routes/webhooks.customers.update.tsx`](app/routes/webhooks.customers.update.tsx) upserts **one Klaviyo API call per mapped metafield**. If the webhook body has no inline `metafields` (common), it **loads `counterpoint` / `klaviyo` metafields via the Admin API** instead (see [`app/lib/klaviyoPropertyMap.ts`](app/lib/klaviyoPropertyMap.ts)).
